        ...
        for item in self.data:
            ...
            if not all(el ==0 for el in itch):
                ...
                for i in range(0,max(itch)):
                    ...
                    childitem=['' if itch[j] ==0 or itch[j]<i else item[j][i] if not isinstance(item[j][i],list) else item[j][i][0][:item[j][i][0].rfind('.')-1]+'.'+item[j][i][0][item[j][i][0].rfind('.'):] for j in range(0,len(item))]
